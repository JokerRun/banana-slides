...
# Core Objective:
[图2]套用[图1]的PPT模板版式，在严格保留[图2]内容信息与业务逻辑的前提下，重新设计页面的信息架构、视觉层级、空间关系与排版方式，输出具有麦肯锡 / BCG 咨询报告风格的专业商务 PPT 页面。
...
- 将 [图1] 的版式框架，严格应用到图 2 上
- 彻底移除 [图 2] 中的原有背景、页眉、页脚、页码、装饰线条、低质量图形、无意义色块、旧版式的视觉干扰元素。
- 仅保留 [图 2] 的原始文本内容、数据信息、业务逻辑。



## 2. 零重写内容原则 (Zero Text Modification)
- 严格保留 [图 2] 的全部文字内容与逻辑层级。 
- 绝对禁止：修改、新增、删除、总结或重写任何文本。 
- 仅允许调整内容的布局位置、对齐方式、字号层级和视觉排版。


## 3. 内容逻辑理解与结构重组 (Structural Override - Critical)
- 强制视觉去噪：[图 2] 为排版草稿。绝对禁止复刻原图中的遮挡块、涂抹痕迹、多余占位符或错位元素。
...
## 4. 标题格式与定位规范
- 条件触发：仅当 [图 2] 原本存在标题时应用；若无标题，严禁新增。 
- 字体规范：微软雅黑 Bold，32pt，板岩蓝 (#3D4F5F)。 
- 布局关系：左对齐贴近内容区左侧。


## 5. 色系规范 (Strict Color Palette)
- 强制色系统一：禁止继承 [图2] 原有颜色。所有图形、金字塔、流程图、图标、模块底色、描边与强调元素，必须重新着色并统一替换为以下规范色系。
...
- 【智能补偿】：若 [图 2] 文本表达缺乏视觉支撑，需深度理解文字语义，自动生成并补充与内容高度匹配的“扁平化图标”或“配图”。补充的图形必须视觉语义准确，且边缘清晰、无复杂的融合式背景。图标间距必须保持统一，且严禁与文字重叠。
