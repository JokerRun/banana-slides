...
def create_app():
    ...
    app.config["IMAGE_PROVIDER_FORMAT"] = os.getenv(
        "IMAGE_PROVIDER_FORMAT", app.config.get("AI_PROVIDER_FORMAT", "gemini")
    )
    ...
    if supports_credentials and cors_origins == "*":
        logging.warning(
            'CORS_ORIGINS="*" is invalid when credentials are enabled, fallback to http://localhost:3000'
        )
    ...
    def get_output_language():
        ...
        return {
            "data": {
                "language": app.config.get("OUTPUT_LANGUAGE", Config.OUTPUT_LANGUAGE)
            }
        }
...
def _load_settings_to_config(app):
    ...
    try:
        ...
        if settings.ai_provider_format:
            ...
            logging.info(
                f"Loaded AI_PROVIDER_FORMAT from settings: {settings.ai_provider_format}"
            )
        ...
        logging.info(
            f"Loaded image settings: {settings.image_resolution}, {settings.image_aspect_ratio}"
        )
        ...
        logging.info(
            f"Loaded worker settings: desc={settings.max_description_workers}, img={settings.max_image_workers}"
        )
        ...
        if settings.mineru_api_base:
            ...
            logging.info(
                f"Loaded MINERU_API_BASE from settings: {settings.mineru_api_base}"
            )
        ...
        if settings.image_caption_model:
            ...
            logging.info(
                f"Loaded IMAGE_CAPTION_MODEL from settings: {settings.image_caption_model}"
            )
        ...
        if settings.output_language:
            ...
            logging.info(
                f"Loaded OUTPUT_LANGUAGE from settings: {settings.output_language}"
            )
        ...
        logging.info(
            f"Loaded reasoning config: text={settings.enable_text_reasoning}(budget={settings.text_thinking_budget}), image_thinking_level={settings.image_thinking_level}"
        )
...
def _preflight_env_or_raise() -> None:
    ...
    image_provider = (
        (os.getenv("IMAGE_PROVIDER_FORMAT") or provider)
        .strip()
        .lower()
        .replace("-", "_")
    )
    if image_provider not in {"openai", "gemini"}:
        raise ValueError(
            f'IMAGE_PROVIDER_FORMAT must be set to "openai" or "gemini" (got {image_provider!r})'
        )

    if provider == "openai":
        if not (os.getenv("OPENAI_API_KEY") or "").strip():
            raise ValueError(
                "OPENAI_API_KEY is required when AI_PROVIDER_FORMAT=openai"
            )
    else:
        if not (os.getenv("GOOGLE_API_KEY") or "").strip():
            raise ValueError(
                "GOOGLE_API_KEY is required when AI_PROVIDER_FORMAT=gemini"
            )

    if image_provider == "openai":
        image_backend = (
            (os.getenv("OPENAI_IMAGE_BACKEND") or "proxy")
            .strip()
            .lower()
            .replace("-", "_")
        )
        image_mode = (
            (os.getenv("OPENAI_IMAGE_MODE") or "responses")
            .strip()
            .lower()
            .replace("-", "_")
        )
        if image_backend not in {"proxy", "azure", "chatgpt"}:
            raise ValueError(
                'OPENAI_IMAGE_BACKEND must be set to "proxy", "azure", or "chatgpt"'
            )
        ...
        if image_mode == "chat" and image_backend != "proxy":
            raise ValueError(
                "OPENAI_IMAGE_MODE=chat is only supported when OPENAI_IMAGE_BACKEND=proxy"
            )
        if (
            image_backend != "chatgpt"
            and not (os.getenv("OPENAI_API_KEY") or "").strip()
        ):
            raise ValueError(
                "OPENAI_API_KEY is required when IMAGE_PROVIDER_FORMAT=openai"
            )
        if image_backend == "chatgpt" and not (
            (os.getenv("OPENAI_API_KEY") or "").strip()
            or (os.getenv("OPENAI_AUTH_JSON") or "").strip()
        ):
            raise ValueError(
                "OPENAI_API_KEY or OPENAI_AUTH_JSON is required when OPENAI_IMAGE_BACKEND=chatgpt"
            )
        if image_mode == "responses":
            if not (os.getenv("OPENAI_RESPONSES_MODEL") or "").strip():
                raise ValueError(
                    "OPENAI_RESPONSES_MODEL is required when OPENAI_IMAGE_MODE=responses"
                )
            if not (os.getenv("OPENAI_IMAGE_MODEL") or "").strip():
                raise ValueError(
                    "OPENAI_IMAGE_MODEL is required when OPENAI_IMAGE_MODE=responses"
                )
        if image_backend == "azure":
            if not (os.getenv("OPENAI_API_BASE") or "").strip():
                raise ValueError(
                    "OPENAI_API_BASE is required when OPENAI_IMAGE_BACKEND=azure"
                )
            if not (os.getenv("OPENAI_IMAGE_DEPLOYMENT") or "").strip():
                raise ValueError(
                    "OPENAI_IMAGE_DEPLOYMENT is required when OPENAI_IMAGE_BACKEND=azure"
                )
