...
class AIService:
    ...
    def __init__(
        self, text_provider: TextProvider = None, image_provider: ImageProvider = None
    ):
        """
        ...
        if has_app_context() and current_app and hasattr(current_app, "config"):
            ...
            self.image_thinking_level = current_app.config.get(
    ...
    def generate_image_prompt(
        ...
        normalized_page_desc, image_refs = self.normalize_markdown_image_references(
            page_desc
        )
    ...
    def edit_restyle_image_with_context(
        self, context, aspect_ratio="16:9", resolution="2K", trace_context=None
    ):
        """
        ...
        conversation_supported = getattr(
            self.image_provider, "supports_conversation_contents", False
        )
        conversation_fallback_allowed = getattr(
            self.image_provider, "allow_conversation_fallback", True
        )
        ...
        if conversation_supported:
            ...
            conversation_request_event = {
                ...
                "conversation_contents": serialize_conversation_contents(
                    context.conversation_contents
                ),
            }
            log_restyle_edit_event(
                "restyle_edit_provider_request",
                trace,
                {
                    "context_mode": "restyle_conversation",
                    "turns_summary": context.turns_summary,
            ...
            try:
                resolved_contents = self._resolve_conversation_images(
                    context.conversation_contents
                )
                ...
                if result:
                    ...
                    log_restyle_edit_event(
                        "restyle_edit_provider_result", trace, result_event
                    )
                    maybe_write_debug_artifact(
                        config,
                ...
                if classified_retryable and conversation_fallback_allowed:
                    logger.warning(
                        f"Conversation mode failed with retryable error, falling back to legacy: {e}"
                    )
                    ...
                    log_restyle_edit_event(
                        "restyle_edit_provider_fallback", trace, fallback_event
                    )
                    maybe_write_debug_artifact(
                        config,
                    ...
                    log_restyle_edit_event(
                        "restyle_edit_provider_result", trace, error_event
                    )
                    maybe_write_debug_artifact(
                        config,
                        event_name="provider_result",
