...
def _allowed_style_ref_file(filename: str) -> bool:
    return (
        "." in filename
        and filename.rsplit(".", 1)[1].lower() in ALLOWED_STYLE_REF_EXTENSIONS
    )
...
def _get_project_reference_files_content(project_id: str) -> list:
    ...
    reference_files = ReferenceFile.query.filter_by(
        project_id=project_id, parse_status="completed"
    ).all()
...
def list_projects():
    ...
    try:
        ...
        projects_with_extra = (
            Project.query.options(joinedload(Project.pages))
        ...
        return success_response(
            {
                "projects": [
                    project.to_dict(include_pages=True) for project in projects
                ],
                "has_more": has_more,
                "limit": limit,
                "offset": offset,
...
def upload_style_refs(project_id):
    ...
    try:
        ...
        replace_existing = (request.form.get("replace") or "").strip().lower() in (
            "1",
            "true",
            "yes",
        )
        ...
        return success_response(
            {
                ...
                "style_ref_image_urls": [
                    f"/files/{path}" for path in project.get_style_ref_image_paths()
                ],
            }
        )
    ...
    try:
        ...
        if "pages_order" in data:
            ...
            pages_to_update = Page.query.filter(
                Page.id.in_(pages_order), Page.project_id == project_id
            ).all()
...
def generate_outline(project_id):
    ...
    try:
        ...
        if reference_files_content:
            logger.info(
                f"Found {len(reference_files_content)} reference files for project {project_id}"
            )
        ...
        if project.creation_type == "outline":
            ...
            if not project.description_text:
                return bad_request(
                    "description_text is required for descriptions type project"
                )

            project_context = ProjectContext(project, reference_files_content)
            outline = ai_service.parse_description_to_outline(
        ...
        for i, page_data in enumerate(pages_data):
            ...
            page.set_outline_content(
                {"title": page_data.get("title"), "points": page_data.get("points", [])}
            )
        ...
        # Return pages
        return success_response({"pages": [page.to_dict() for page in pages_list]})
...
def generate_from_description(project_id):
    ...
    try:
        ...
        if len(pages_data) != len(page_descriptions):
            logger.warning(
                f"页面数量不匹配: 大纲 {len(pages_data)} 页, 描述 {len(page_descriptions)} 页"
            )
            # 取较小的数量，避免索引错误
        ...
        logger.info(
            f"从描述生成完成: 项目 {project_id}, 创建了 {len(pages_list)} 个页面，已填充大纲和描述"
        )
...
def generate_descriptions(project_id):
    ...
    try:
        ...
        # Get pages
        pages = (
            Page.query.filter_by(project_id=project_id).order_by(Page.order_index).all()
        )
        ...
        max_workers = data.get(
            "max_workers", current_app.config.get("MAX_DESCRIPTION_WORKERS", 5)
        )
        language = data.get("language", current_app.config.get("OUTPUT_LANGUAGE", "zh"))
...
def generate_images(project_id):
    ...
    try:
        ...
        pages = get_filtered_pages(
            project_id, selected_page_ids if selected_page_ids else None
        )
...
def get_task_status(project_id, task_id):
    ...
    try:
        ...
        task = Task.query.filter_by(
            id=task_id, project_id=project_id, owner_id=get_current_user_id()
        ).first()
...
def refine_outline(project_id):
    ...
    try:
        ...
        if reference_files_content:
            logger.info(
                f"Found {len(reference_files_content)} reference files for refine_outline"
            )
            for rf in reference_files_content:
                logger.info(f"  - {rf['filename']}: {len(rf['content'])} characters")
        else:
        ...
        # Refine outline
        logger.info(
            f"开始修改大纲: 项目 {project_id}, 用户要求: {user_requirement}, 历史要求数: {len(previous_requirements)}"
        )
        ...
        logger.info(
            f"描述匹配完成: 保留了 {preserved_count} 个页面的描述, {new_count} 个页面需要重新生成描述"
        )
...
def refine_descriptions(project_id):
    ...
    try:
        ...
        if not pages:
            ...
            return bad_request(
                "No pages found for project. Please generate outline first."
            )
        ...
        if reference_files_content:
            logger.info(
                f"Found {len(reference_files_content)} reference files for refine_descriptions"
            )
            for rf in reference_files_content:
                logger.info(f"  - {rf['filename']}: {len(rf['content'])} characters")
        else:
        ...
        # Refine descriptions
        logger.info(
            f"开始修改页面描述: 项目 {project_id}, 用户要求: {user_requirement}, 历史要求数: {len(previous_requirements)}"
        )
        ...
        if len(refined_descriptions) != len(pages):
            error_msg = ""
            logger.error(
                f"AI 返回的描述数量不匹配: 期望 {len(pages)} 个页面，实际返回 {len(refined_descriptions)} 个描述。"
            )
        ...
        # Return pages
        return success_response(
            {"pages": [page.to_dict() for page in pages], "message": "页面描述修改成功"}
        )
