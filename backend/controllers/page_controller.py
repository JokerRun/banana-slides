...
def _get_owned_page(project_id: str, page_id: str):
    return (
        Page.query.join(Project, Page.project_id == Project.id)
        .filter(
            Page.id == page_id,
            Page.project_id == project_id,
            Project.owner_id == get_current_user_id(),
        )
        .first()
    )
...
def create_page(project_id):
    ...
    try:
        ...
        other_pages = Page.query.filter(
            Page.project_id == project_id, Page.order_index >= data["order_index"]
        ).all()
...
def delete_page(project_id, page_id):
    ...
    """
    try:
        page = _get_owned_page(project_id, page_id)
...
def generate_page_description(project_id, page_id):
    ...
    try:
        ...
        if page.get_description_content() and not force_regenerate:
            return bad_request(
                "Description already exists. Set force_regenerate=true to regenerate"
            )
        ...
        desc_text = ai_service.generate_page_description(
            project_context, outline, page_data, page.order_index + 1, language=language
        )
...
def generate_page_image(project_id, page_id):
    ...
    try:
        ...
        if page.generated_image_path and not force_regenerate:
            return bad_request(
                "Image already exists. Set force_regenerate=true to regenerate"
            )
        ...
        elif not style_ref_paths and not ref_image_path and not project.template_style:
        ...
        return success_response(
            {"task_id": task.id, "page_id": page_id, "status": "PENDING"},
            status_code=202,
        )
...
def get_page_image_versions(project_id, page_id):
    ...
    try:
        ...
        return success_response({"versions": [v.to_dict() for v in versions]})
...
def set_current_image_version(project_id, page_id, version_id):
    ...
    try:
        ...
        version = PageImageVersion.query.filter_by(
            id=version_id, page_id=page_id
        ).first()
        ...
        file_service = FileService(current_app.config["UPLOAD_FOLDER"])
        cached_relative_path = file_service.get_cached_image_path(
            project_id, page_id, version.version_number
        )
