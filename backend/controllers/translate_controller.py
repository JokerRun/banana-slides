...
def create_translate_project():
    ...
    - style_refs: File[] (1-N style reference images) — optional, required for "restyle" mode
    ...
    try:
        ...
        if translate_mode == 'restyle' and (not style_refs or len(style_refs) == 0):
            return bad_request("At least one style reference image is required for restyle mode")
        ...
        logger.info(f"✅ Translate project created: id={project.id}, name='{source_name}', "
...
@translate_bp.route('/<project_id>/pages/<page_id>/translate/generate', methods=['POST'])
