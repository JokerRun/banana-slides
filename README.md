...
- **布局建议**：页面描述会自动附带 ASCII Diagram 版式建议，用于引导后续页面生成；该建议不会作为页面文字渲染。
...
1. **生成页面文字有乱码，文字不清晰**
    ...
    - 页面描述里的 `布局建议（Layout Recommendation - ASCII Diagram）` 只用于排版指导，不会作为幻灯片正文渲染。
...
6. **配置入口在哪里？为什么找不到设置页？**
    - 当前版本使用 env-only 配置模式，前端已移除 settings 页面。
    - 所有全局配置仅支持通过 `.env`/部署密钥管理修改，并通过重启服务生效。

## 🗺️ 开发计划

| 状态 | 里程碑 |
| --- | --- |
| ✅ 已完成 | 从想法、大纲、页面描述三种路径创建 PPT，并为页面描述生成 ASCII 版式建议 |
...
├── docs/                       # 内部文档
│   ├── superpowers/            # 规格与实现计划（当前权威设计文档）
│   ├── guides/                 # 操作与调试指南
│   ├── requirements/           # 需求与原型材料
│   └── lavish/                 # Lavish 历史 HTML 报告（索引：docs/lavish/README.md，实施前以代码为准）
├── tests/                      # 测试文件目录
├── v0_demo/                    # 早期演示版本
├── output/                     # 输出文件目录
│
